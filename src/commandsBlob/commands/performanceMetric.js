...
const commandsMetaData = [
  {[wrd.cName]: cmd.cbusinessRulesMetrics, [sys.cFilePath]: filePath, [wrd.cthreadable]: false, [sys.ccommandsDependencies]: [], [sys.cbusinessRulesDependencies]: [
    biz.carrayDeepClone
  ]},
  {[wrd.cName]: cmd.ccommandMetrics, [sys.cFilePath]: filePath, [wrd.cthreadable]: false, [sys.ccommandsDependencies]: [], [sys.cbusinessRulesDependencies]: [
    biz.carrayDeepClone
  ]}
];

/**
 * @function initPerformanceMetric
 * @description Adds the performanceMetric commands meta-data to the D-data structure commandsMetaData-framework data structure.
 ...
 * @date 2025/07/17
 */
async function initPerformanceMetric() {
  const functionName = initPerformanceMetric.name;
...
async function businessRulesMetrics(inputData, inputMetaData) {
  const functionName = businessRulesMetrics.name;
  await loggers.consoleLog(namespacePrefix + functionName, msg.cBEGIN_Function);
...
async function commandMetrics(inputData, inputMetaData) {
  const functionName = commandMetrics.name;
  await loggers.consoleLog(namespacePrefix + functionName, msg.cBEGIN_Function);
  await loggers.consoleLog(namespacePrefix + functionName, msg.cinputDataIs + JSON.stringify(inputData));
  await loggers.consoleLog(namespacePrefix + functionName, msg.cinputMetaDataIs + inputMetaData);
  let returnData = [true, []];
